...
本工具允許玩家在不解碼、不替換原始巨大檔案的情況下，透過外部掛載的方式載入 .asar 格式的語言包、修正檔或功能模組。此架構能大幅減少補丁體積，並更好地保護遊戲作者以及模組創作者的著作權。
...
取得您想要使用的模組檔案（通常是 .asar 格式，例如 zh-tw.asar）。

將該檔案放入遊戲根目錄下的 plugins 資料夾中。
...
01_Base_Translation.asar (基礎漢化)

99_UI_Fix.asar (介面修正)

載入結果：
Loader 會先讀取 99_UI_Fix.asar，然後再讀取 01_Base_Translation.asar。

如果兩個模組都修改了同一張圖片，遊戲將會顯示 99_UI_Fix.asar 裡面的版本。
這讓您可以輕鬆製作「修正檔」或「相容包」，而不需要重新打包整個 asar 檔。
